...
export default function Inquiry_home(props){
    ...
    function render(s){
        let {item} = s
        let {title,status,date,subjectID,id} = item
        ...
        return(
        <LongBox heading={title} lg colors={gradient} text={`${subject} | Date: ${date}`} fun={()=>{navigation.navigate('inquiry',{data:{id},deleteFun:handleDelete})}} icon="document-outline"/>
        )
    }
